...
namespace LN {
  ...
  LongNumber& LongNumber::operator+=(const LongNumber& value) {
    ...
    if (sign == val.sign) {
      int over = 0;
      int idx = 0;
      while (idx < std::max((int)digits.size(), (int)val.digits.size()) || over != 0) {
  ...
  bool LongNumber::cmp_abs(const LongNumber& bi1, const LongNumber& bi2) {
    if ((int)bi1.digits.size() - bi1.exponent != (int)bi2.digits.size() - bi2.exponent) {
      return (int)bi1.digits.size() - bi1.exponent > (int)bi2.digits.size() - bi2.exponent;
    }
  ...
  LongNumber& LongNumber::operator/=(const LongNumber& value) {
    ...
    for (int i = (int)digits.size() - 2; i >= (int)val.digits.size() - 1; i--) {
      ...
      while (r - l > 1) {
        ...
        if (kol == (int)tec.digits.size() || digits[i + 1 - kol] > tec.digits[(int)tec.digits.size() - 1 - kol]) {
          l = m;
        }
        else {
          r = m;
  ...
  LongNumber::operator bool() {
    ...
    if ((int)val1.digits.size() != (int)val2.digits.size() || val1.sign != val2.sign) {
        return false;
    }
